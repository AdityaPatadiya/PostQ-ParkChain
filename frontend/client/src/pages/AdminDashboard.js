...
import "../css/AdminDashboard.css"; // Import CSS
...
const AdminDashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const totalAssigned = Object.values(formData.slots).reduce(
      (sum, s) => sum + s.count,
      0
    );
    ...
    if (totalAssigned > formData.totalSlots) {
      alert(
        `Total assigned slots (${totalAssigned}) exceed total available slots (${formData.totalSlots}).`
      );
      return;
    }
    ...
    try {
      await addDoc(collection(db, "parkingPlots"), {
        ...
        remainingSlots: formData.totalSlots - totalAssigned,
        createdAt: new Date(),
      });
  ...
  return (
    <div className="container">
      <h2 className="text-center">Admin Panel - Add Parking Plot</h2>
              ...
              <div key={slot.key} className="mb-2 row">
                <div className="col-4">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleChange("slots", slot.key, "count", e.target.value)
                    }
                  />
                </div>
                <div className="col-4">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleChange("slots", slot.key, "price", e.target.value)
                    }
                  />
                </div>
              </div>
